...
static const uint col_pins[NUM_COLS] = {0, 1, 2};
static const uint row_pins[NUM_ROWS] = {3, 6, 7};
...
/*
 * Diodes conduct column -> row. Strobe one column high and read the rows
 * (pulled down). A closed switch pulls that row high.
 */
static void matrix_scan(bool pressed[NUM_ROWS][NUM_COLS]) {
    for (int c = 0; c < NUM_COLS; c++) {
...
int main(void) {
    ...
    bool last_pressed[NUM_ROWS][NUM_COLS] = {0};
    matrix_scan(last_pressed);
    ...
    while (true) {
        ...
        for (int r = 0; r < NUM_ROWS; r++) {
            for (int c = 0; c < NUM_COLS; c++) {
                if (!last_pressed[r][c] && current[r][c]) {
                    sleep_ms(DEBOUNCE_MS);
                    matrix_scan(current);
                    if (current[r][c]) {
                        char msg[24];
                        snprintf(msg, sizeof(msg), "Button %d clicked!", keymap[r][c]);
                        oled_show_message(msg);
                    }
                }
                last_pressed[r][c] = current[r][c];
            }
        }
